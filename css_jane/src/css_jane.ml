...
(* module Shunting_Yard_Algorith = struct
  let shunting_yard_algorith list_tokens = 
    let input = Queue.create () in
    let tokens = Queue.create () in
    let output = Queue.create () in
    let operators = Stack.create () in
    let combinators = ['>' ; '+' ; '~' ; [ '|' '|' ]] in
    let start_number = -1 in

    while (not List.length list_tokens == 0)
      do 
      let token = list_tokens[start_number + 1] in
      Queue.push input token ; 

      for tokens_queue = 0 to (List.length list_tokens)
       Queue.push 

     *)


  (* module Preference Algorithm = struct
  let algorith token = 
    let operator_stack = Stack.create () in 
    let result_stack = Stack.create () in
  if 
